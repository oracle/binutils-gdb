...
static const struct objfile_data *ctf_dict_key;
...
/* Fetch the type for TID in OF's tid_and_type hash, add the type to
   context CCP if hash is empty or TID does not have a saved type.  */

static struct type *
fetch_tid_type (struct ctf_context *ccp, struct objfile *of, ctf_id_t tid)
{
  struct type *typ;
		   ...
		   void *arg)
{
  ...
  t = fetch_tid_type (ccp, ccp->of, tid);
  if (t == NULL)
    {
...
read_func_kind_type (struct ctf_context *ccp, ctf_id_t tid)
{
  ...
  if (argc != 0)
	...
	= (struct field *) TYPE_ZALLOC (type, argc * sizeof (struct field));
      ...
      /* TYPE_FIELD_TYPE must never be NULL.  If failed to find the
	 argument type, fill it with void_type.  */
      for (int iparam = 0; iparam < argc; iparam++)
	{
	  atype = get_tid_type (of, argv[iparam]);
	  if (atype)
	    TYPE_FIELD_TYPE (type, iparam) = atype;
	  else
	    TYPE_FIELD_TYPE (type, iparam) = void_type;
	}
      xfree (argv);
    }
...
read_array_type (struct ctf_context *ccp, ctf_id_t tid)
{
  ...
  element_type = fetch_tid_type (ccp, objfile, ar.ctr_contents);
  if (element_type == NULL)
    return NULL;

  idx_type = fetch_tid_type (ccp, objfile, ar.ctr_index);
  if (idx_type == NULL)
...
read_const_type (struct ctf_context *ccp, ctf_id_t tid, ctf_id_t btid)
{
  ...
  base_type = fetch_tid_type (ccp, objfile, btid);
  if (base_type == NULL)
    {
		   ...
		   ctf_id_t btid, const char *name)
{
  ...
  target_type = fetch_tid_type (ccp, objfile, btid);
...
read_pointer_type (struct ctf_context *ccp, ctf_id_t tid, ctf_id_t btid)
{
  ...
  target_type = fetch_tid_type (ccp, of, btid);
  if (target_type == NULL)
    {
...
add_stt_obj (struct ctf_context *ccp, unsigned long idx)
{
  ...
  type = fetch_tid_type (ccp, ccp->of, tid);
  if (type == NULL)
    return NULL;
...
add_stt_func (struct ctf_context *ccp, unsigned long idx)
{
  ...
  ftype = fetch_tid_type (ccp, ccp->of, tid);
  ...
  for (int iparam = 0; iparam < argc; iparam++)
    {
      atyp = fetch_tid_type (ccp, ccp->of, argv[iparam]);
      if (atyp)
  ...
  rettyp = fetch_tid_type (ccp, ccp->of, finfo.ctc_return);
  if (rettyp != NULL)
...
get_objfile_text_range (struct objfile *of, int *tsize)
{
  ...
  *tsize = codes ? bfd_section_size (abfd, codes) : 0;
...
ctf_read_symtab (struct partial_symtab *pst, struct objfile *objfile)
{
  ...
  if (pst->readin)
      ...
      if (info_verbose)
	{
      ...
      /* Start a symtab.  */
      CORE_ADDR text_base;        /* Start of text segment.  */
      int tsize;

      text_base = get_objfile_text_range (objfile, &tsize);
      start_symtab (objfile, objfile->original_name, NULL,
		    text_base, language_c);
      start_subfile (objfile->original_name);
      record_debugformat ("ctf");
      psymtab_to_symtab (pst);

      pst->textlow = gdbarch_adjust_dwarf2_addr (gdbarch, text_base);
      pst->texthigh = gdbarch_adjust_dwarf2_addr (gdbarch, text_base + tsize);
      pst->compunit_symtab = end_symtab (text_base + tsize,
...
{
  ctf_dict_t *fp = static_cast<ctf_dict_t *> (datum);
...
elfctf_build_psymtabs (struct objfile *of)
{
  ...
  if (fp == NULL)
    error (_("ctf_dict_open failed on %s - %s"),
	   bfd_get_filename (abfd), ctf_errmsg (err));
  set_objfile_data (of, ctf_dict_key, fp);
...
_initialize_ctfread ()
{
  ctf_tid_key = register_objfile_data ();
  ctf_dict_key
