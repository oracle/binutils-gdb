...
/* Final link: output is not expected to be fed back into ctf_link again.  */
#define CTF_LINK_FINAL 0x2

/* Do a nondeduplicating link.  */
#define CTF_LINK_NONDEDUP 0x4

/* Nonreproducible link: faster, but types may be emitted in different orders if
   linked repeatedly.  */
#define CTF_LINK_NONREPRODUCIBLE 0x8

/* Create empty outputs for all registered CU mappings even if no types are
   emitted into them.  */
#define CTF_LINK_EMPTY_CU_MAPPINGS 0x10
