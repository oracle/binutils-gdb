                ...
                const char *s4, char *s5, const char *s6)
{
  ...
  snprintf (buf, bufsiz, GTXT ("%-*s %-*s%c%*s%*s %-*s %s"),
...
hwc_usage_internal (int forKernel, FILE *f_usage, const char *cmd, const char *dataspace_msg, int show_syntax, int show_short_desc)
{
  ...
  if (show_syntax)
    {
      if (has_attrs)
        fprintf (f_usage, GTXT ("    <ctr_def> == <ctr>[[~<attr>=<val>]...][/<reg#>],[<rate>]\n"));
      else
        fprintf (f_usage, GTXT ("    <ctr_def> == <ctr>[/<reg#>],[<rate>]\n"));
      if (dataspace_msg)
  ...
  if (show_syntax)
    {
      ...
      fprintf (f_usage, GTXT ("           forces use of a specific hardware register.  (Solaris only)\n"));
      fprintf (f_usage, GTXT ("           If not specified, %s will attempt to place the counter into the first\n"
                              "           available register and as a result may be unable to place\n"
                              "           subsequent counters due to register conflicts.\n"), cmd);
      fprintf (f_usage, GTXT ("           The / in front of the register number is required if a register is specified.\n\n"));
  ...
  if (strlen (tmp))
    {
      // Check if the message contains obsolete URL http://www.sun.com
      const char * oldURL = NTXT ("http://www.sun.com/processors/manuals");
      const char * newURL = NTXT ("http://www.oracle.com/technetwork/documentation/oracle-sparc-ent-servers-189996.html");
      char * s = strstr (tmp, oldURL);
      if (s != NULL)
        {
          // Replace the URL
          *s = '\0';
