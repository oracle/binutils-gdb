...
{
  cpuid_regs_t cp;
  if (cpuid_getvendor () != X86_VENDOR_Intel)
    return (-1);
  ...
  (void) my_cpuid (0xa, &cp);
  versionid = cp.eax & 0xFF;
  ...
  if (num_ffc >= 64)
    return (-1);
  known_ffc_num = sizeof (ffc_names) / sizeof (char *) - 1; /* -1 for EOT */
