...
DbeView::remove_compare_metrics (MetricList *mlist)
{
  ...
  items->reset ();
  BaseMetric::SubType sort_subtype = BaseMetric::STATIC;
  char *sort_cmd = NULL;
  for (int i = 0, sz = items_old->size (); i < sz; i++)
    {
      Metric *m = items_old->fetch (i);
      if (mlist->get_sort_ref_index () == i)
        {
          sort_cmd = m->get_cmd ();
          sort_subtype = m->get_subtype ();
        }
  ...
  if (sort_cmd)
    {
      // find the new location of the sort column
      sort_idx = mlist->get_listorder (sort_cmd, sort_subtype);
      if (sort_idx < 0)
        sort_idx = 0;
    }
  mlist->set_sort_ref_index (sort_idx);
  delete items_old;
  reset_data (false);
}
