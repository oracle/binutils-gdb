...
#elif ARCH(Aarch64)
#define FILL_CONTEXT(context) getcontext(context)
#endif /* ARCH() */
...
stack_unwind (char *buf, int size, void *bptr, void *eptr, ucontext_t *context, int mode)
{
  ...
  __u64 *sp = (__u64 *) context->uc_mcontext.sp;
  ...
  while (sp && pc)
    {
      ...
      if (!dladdr ((void *) pc, &dlinfo))
        break;
      TprintfT (DBG_LT3, "%2d: %llx <%s+%llu> (%s)\n",
      ...
      if (ind >= lsize)
        break;
      unsigned long tbgn, tend;
      if (!__collector_check_segment ((unsigned long) pc, &tbgn, &tend, 0))
        {
          DprintfT (SP_DUMP_UNWIND, "unwind.c:%d __collector_check_segment failed. sp=0x%lx\n",
