...
/* Iterate over all CTF files in an archive, returning each archive in turn as a
   ctf_file_t.  It is the caller's responsibility to close it.  */
		  ...
		  int *errp)
{
  ...
  /* Iteration is made a bit more complex by the need to handle ctf_file_t's
     transparently wrapped in a single-member archive.  */
  ...
  const ctf_sect_t *symsect = &wrapper->ctfi_symsect;
  const ctf_sect_t *strsect = &wrapper->ctfi_strsect;
