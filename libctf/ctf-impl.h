...
/* Deduplicator state.
   ...
   ctf_dedup_hash_type; a "type ID" is not what it is in the rest of libctf but
   rather is a a packed-together reference to a single ctf_file_t and ctf_id_t,
   i.e. a single instance of some hash value in some input.

   The grouping and deduplication algorithms take a bunch of inputs and yield a
   single shared "output" and possibly many outputs corresponding to individual
   inputs that still contain types after sharing of unconflicted types.  Almost
   all deduplicator state is stored in the output, though a (very) few things
   are stored in inputs for simplicity's sake, usually if they are linking
   together things within the scope of a single TU.
...
typedef struct ctf_dedup
{
  ...
  /* On 32-bit platforms only, a hash of type IDs, in the form of
     a ctf_link_type_key_t.  */
  ctf_dynhash_t *cd_id_to_file_t;

  /* Map type names to a hash from type hash value -> number of times each value
     has appeared.  The hash values are initially "forward-equivalent", i.e. all
     structs/unions/enums are collapsed to forwards.  */
  ctf_dynhash_t *cd_name_hashes;

  /* Map type names to the hash value of the forward with that name, if any.
     Only forwards corresponding to names that are not conflicting are
     guaranteed to be recorded here, so make sure all uses are compatible with
     that!

     Populated fairly late, by ctf_dedup_detect_name_ambiguity.  */
  ctf_dynhash_t *cd_name_forwards;

  /* Map type IDs to type hash values.  Used to determine if types are already
     hashed without having to recompute their hash values again, and to link
     types together at later stages.

     Initially forward-equivalent: wiped after forward-equivalency is no longer
     needed.  */
  ctf_dynhash_t *cd_type_hashes;

  /* Maps from the names of structs/unions/enums to a set (a hash) of hash
     values of all structs/unions/enums with that name.  Used to unify structs
     that contain forwards in some TUs but real structs in others.  */
  ctf_dynhash_t *cd_real_structs_set;

  /* Like cd_real_structs_set, but contains only those names that map to a
     unique, unconflicting struct/union/enum: the value is a single hash value.
     Discarded when the forward-elision mapping (below) is populated.  */
  ctf_dynhash_t *cd_real_structs;

  /* A set tracking the hash values of forwards to types that are marked as
     conflicting.  */
  ctf_dynhash_t *cd_forwards_to_conflicting;

  /* Tracks the hash values of forwards to single, unambiguous concrete types: A
     mapping from type hash value to type hash value.  */
  ctf_dynhash_t *cd_forward_unification;

  /* Maps type hashes to input type IDs.  The value is a set (a hash) of type
     IDs.  Initially forward-equivalent: wiped after forward-equivalency is no
     longer needed.  The result of the ctf_group function.  */
  ctf_dynhash_t *cd_output_mapping;

  /* Maps the type IDs of structures in input TUs whose members still need
     emission to the type ID of the already-emitted target type (which has no
     members yet) in the appropriate target.  Uniquely, the latter type ID
     represents a *target* ID (i.e. the cd_output_mapping of some specified
     input): we encode the shared (parent) dict with an ID of -1.  */
  ctf_dynhash_t *cd_emission_struct_members;

  /* A set (a hash) of type IDs of conflicted types during FE-hashing.  */
  ctf_dynhash_t *cd_conflicted_type_ids;

  /* A set (a hash) of hash values of conflicted types.  Transferred from
     cd_conflicted_type_ids after FE-hashing is over.  */
  ctf_dynhash_t *cd_conflicted_types;
  ...
  ctf_dynhash_t *cd_emission_hashes;
...
struct ctf_next
{
  ...
  /* We can save space on this side of things by noting that a container is
     ...
     over one kind of things at once: so we can overlap the DTD and non-DTD
  ...
  union
  {
    ...
    ctf_next_hkv_t *ctn_hkv;
  } u;
  /* This union is of various sorts of container we can iterate over:
     currently dictionaries, archives, and dynhashes.  */
  union
  {
...
extern void *ctf_dynhash_iter_find (ctf_dynhash_t *, ctf_hash_iter_find_f,
				    void *);
extern int ctf_dynhash_next (ctf_dynhash_t *, int sorted, ctf_next_t **,
			     void **key, void **value);
...
extern int ctf_dedup_group (ctf_file_t *, ctf_file_t **, uint32_t,
			    int cu_mapped);
extern ctf_file_t **ctf_dedup_emit (ctf_file_t *, ctf_file_t **, uint32_t,
				    uint32_t *noutputs, int cu_mapped);
extern void ctf_dedup_group_fini (ctf_file_t *);
...
ctf_new_archive_internal (int is_archive, struct ctf_archive *,
			  ctf_file_t *, const ctf_sect_t *symsect,
