...
ctf_next_destroy (ctf_next_t *i)
{
  ...
  if (i->ctn_iter_fun == (void (*) (void)) ctf_dynhash_next)
    free (i->u.ctn_hkv);
  free (i);
}
...
/* Traverse a dynhash, in iterator form.  */

static void
ctf_dynhash_next_accum_kvs (void *key, void *value, void *arg)
{
  ctf_next_t *i = (ctf_next_t *) arg;

  /* Note that this stashes all the kv pairs in reverse order.  It's easy to
     compensate for that, and avoids us having to add even more variables to the
     ctf_next_t.  */
  assert (i->ctn_n > 1);

  i->ctn_n--;
  i->u.ctn_hkv[i->ctn_n - 1].hkv_key = key;
  i->u.ctn_hkv[i->ctn_n - 1].hkv_value = value;
}

/* Sort the keys in the array of keys used by ctf_dynhash_next into order.  */
static int
ctf_dynhash_next_sort_kvs (const void *one, const void *two)
{
  ctf_next_hkv_t *a = (ctf_next_hkv_t *) one;
  ctf_next_hkv_t *b = (ctf_next_hkv_t *) two;

  return strcmp (a->hkv_key, b->hkv_key);
}

/* Note: unusually, this returns *negative error values*, because it does not
   take an fp, and taking an error pointer would be incredibly clunky.

   If SORTED is t, sort keys before iterating over them: otherwise, iterate in
   random order.  */
int
ctf_dynhash_next (ctf_dynhash_t *h, int sorted, ctf_next_t **it, void **key,
		  void **value)
{
  ctf_next_t *i = *it;
  ...
  if (!i)
	...
	return -ENOMEM;
      ...
      if ((i->u.ctn_hkv = calloc (els, sizeof (ctf_next_hkv_t))) == NULL)
	{
	  ctf_next_destroy (i);
	  return -ENOMEM;
	}
      ...
      i->cu.ctn_h = h;
      /* We have to work using a 1-based array count so that we can use 0 to
	 indicate iteration completion.  */
      i->ctn_n = els + 1;
      ctf_dynhash_iter (h, ctf_dynhash_next_accum_kvs, i);
      if (sorted)
	qsort (i->u.ctn_hkv, els, sizeof (ctf_next_hkv_t),
	       ctf_dynhash_next_sort_kvs);
      i->ctn_n = els + 1;
  ...
  if (key)
    *key = i->u.ctn_hkv[i->ctn_n - 1].hkv_key;
  if (value)
    *value = i->u.ctn_hkv[i->ctn_n - 1].hkv_value;
  return 0;
}
