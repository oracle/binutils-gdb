...
init_types (ctf_file_t *fp, ctf_header_t *cth)
{
  ...
  *xp++ = 0;			/* Type id 0 is used as a sentinel value.  */
  ...
  /* Make an additional pass through the pointer table to find pointers that
     ...
     by the anonymous typedef node.  Also hunt for pointers to non-root-visible
     types with names, and make sure that any root-visible instance of that name
     links to this pointer if it doesn't already link to some pointer or
     other.  */
  ...
  for (id = 1; id <= fp->ctf_typemax; id++)
      ...
      if ((dst = fp->ctf_ptrtab[id]) != 0)
	{
	  ...
	  if (!LCTF_INFO_ISROOT (fp, tp->ctt_info) && (tp->ctt_name != 0))
	    {
	      int kind = LCTF_INFO_KIND (fp, tp->ctt_info);
	      ctf_names_t *np = ctf_name_table (fp, kind);
	      ctf_id_t named_type;

	      named_type = ctf_hash_lookup_type (np->ctn_readonly, fp,
						 ctf_strptr (fp, tp->ctt_name));
	      if ((named_type != 0)
		  && LCTF_TYPE_ISCHILD (fp, tp->ctt_type) == child
		  && fp->ctf_ptrtab[LCTF_TYPE_TO_INDEX (fp, named_type)] == 0)
		fp->ctf_ptrtab[LCTF_TYPE_TO_INDEX (fp, named_type)] = dst;
	    }
	}
    }
...
ctf_file_close (ctf_file_t *fp)
{
  ...
  ctf_dedup_group_fini (fp);
  ctf_dynhash_destroy (fp->ctf_dedup_atoms);
